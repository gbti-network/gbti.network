...
class GbtiCategoriesWorkspace extends GbtiElement {
  ...
  get _channelsOn() { return typeof this.client?.contentChannelPool === 'function' && typeof this.client?.discordChannels === 'function'; }
  ...
  _discordHtml(key) {
    if (!this._channelsOn) return ''; // sow-161 A: hidden on the website (no channel reads) until the channel-map increment
  ...
  _wire() {
    ...
    const openNew = () => { this._newOpen = true; this.render(); this.$('#nckey')?.focus(); };
    this.on('#newtop', 'click', openNew);
    this.on('#newtop2', 'click', openNew);
    this.on('#nccancel', 'click', () => { this._newOpen = false; this.render(); });
    this.on('#ncadd', 'click', () => {
      ...
      if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(key)) { if (err) err.textContent = 'The key is lowercase kebab-case (letters, digits, hyphens).'; return; }
      if (flattenTree(this._tree).some((n) => n.key === key)) { if (err) err.textContent = `The key "${key}" already exists in the tree.`; return; }
