...
class GbtiChannelMapManager extends GbtiElement {
  ...
  async load() {
    ...
    try {
      const [channels, flags, templates, engagement, pipeline] = await Promise.all([
        ...
        this.client.syndicationSettings ? this.client.syndicationSettings().catch(() => null) : null, // SOW-088
      ]);
      ...
      this._pipeline = pipeline?.settings || null; // SOW-088: master/approval/hold/channel switches
    } catch {
      this._channels = [];
      this._lists = {};
      this._templates = {};
  ...
  _pipelineHtml() {
    ...
    if (!p) return '';
    const BUILDING = new Set(['x', 'linkedin', 'mastodon', 'bluesky']);
  ...
  async _savePipeline() {
    ...
    if (!p) return;
    const channels = {};
    this.$$('[data-pipe-chan]').forEach((c) => { channels[c.dataset.pipeChan] = c.checked; });
    this._busy = true; this._msg = null; this.render();
    try {
      ...
      this._pipeline = null; // reload picks up the committed values
      this._channels = null;
    } catch (err) {
  ...
  _wire() {
    this.on('[data-pipe-save]', 'click', () => this._savePipeline()); // SOW-088
