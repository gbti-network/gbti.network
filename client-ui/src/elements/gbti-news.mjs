...
class GbtiNews extends GbtiElement {
  connectedCallback() {
    ...
    // SOW-046 D: when a member posts a comment on the OPEN news item, tell the Worker so it appends a one-time
    // "members are discussing this" notice to the curator-posted Discord message. Best-effort + idempotent
    // server-side, so a failure (or an item never posted to Discord) is silently ignored.
    this._onComment = (e) => {
      const it = this._open;
  ...
  _renderReader() {
    ...
    // SOW-046 D: members discuss the item via the shared comment engine. The slug is the deterministic hash of the
    // (URL-shaped) guid; <gbti-discussion> reads the thread + mounts its own paid-gated composer.
    ...
    host.innerHTML = `<div class="rd"><button class="back" data-back type="button">← Back to feed</button>`
      ...
      + `<div class="acts">${src}${disc}</div>${note}${discussion}</div>`;
