...
test('drain: a manual-assist channel enqueues a task and NEVER calls an adapter', async () => {
  const cfg = JSON.stringify({ syndication: { enabled: true, require_approval: false, hold_minutes: 60, channels: { discord: false }, manual_assist_channels: ['x'], auto_matrix: { post: { linkedin: 'off', dailydev: 'off', hashnode: 'off', reddit: 'off', bluesky: 'off' } } } });
...
test('drain: does not re-task a channel already queued-manual on a prior tick', async () => {
  const cfg = JSON.stringify({ syndication: { enabled: true, require_approval: false, hold_minutes: 60, channels: {}, manual_assist_channels: ['x'], auto_matrix: { post: { linkedin: 'off', dailydev: 'off', hashnode: 'off', reddit: 'off', bluesky: 'off' } } } });
  const kv = fakeKV({ [SYND_CONFIG_KEY]: cfg });
...
test('endpoint POST action=post: the adapter posts the reviewed text and the task completes', async () => {
  const kv = fakeKV();
  await putTask(kv, { id: 'a::devto', itemId: 'a', channel: 'devto', source: 'post', title: 'T', url: 'https://e.com/t',
    text: 'Reviewed text', item: { source: 'post', targetSlug: 'a/x', title: 'T', url: 'https://e.com/t' }, status: 'pending', createdAt: 1 });
  const calls = [];
  const adapters = { devto: { name: 'devto', enabled: () => true, post: async (i) => { calls.push(i); return { ok: true, id: 'p1', url: 'https://dev.to/p1' }; } } };
  const res = await handleSocialQueueAction(req({ action: 'post', id: 'a::devto' }),
    { SIGNUP_KV: kv, DEVTO_API_KEY: 'k', DEVTO_ORG_ID: 'o' }, { kv, now: at(9), authorize: superAuth, adapters });
  assert.equal(res.status, 200);
  assert.equal(res.body.posted, true);
  assert.equal(res.body.postedUrl, 'https://dev.to/p1');
  ...
  const done = await getTask(kv, 'a::devto');
  assert.equal(done.status, 'done');
  assert.equal(done.postedVia, 'adapter');
  assert.equal(done.postedUrl, 'https://dev.to/p1');
});
...
test('endpoint POST action=post: manual-capability channels, missing secrets, and failures are refused', async () => {
  ...
  await putTask(kv, { id: 'a::devto', channel: 'devto', text: 't', status: 'pending', createdAt: 1 });
  await putTask(kv, { id: 'b::devto', channel: 'devto', text: 't', status: 'done', doneAt: 2 });
  ...
  // sow-405: nor can Bluesky any more, since it became assisted posting
  await putTask(kv, { id: 'a::bluesky', channel: 'bluesky', text: 't', status: 'pending', createdAt: 1 });
  const rb = await handleSocialQueueAction(req({ action: 'post', id: 'a::bluesky' }), { SIGNUP_KV: kv, BLUESKY_HANDLE: 'h', BLUESKY_APP_PASSWORD: 'p' }, { kv, authorize: superAuth });
  ...
  const rs = await handleSocialQueueAction(req({ action: 'post', id: 'a::devto' }), { SIGNUP_KV: kv }, { kv, authorize: superAuth });
  assert.equal(rs.status, 409);
  assert.equal((await getTask(kv, 'a::devto')).status, 'pending');
  // an adapter failure leaves the task pending and reports the error
  const failing = { devto: { name: 'devto', enabled: () => true, post: async () => ({ ok: false, error: 'boom' }) } };
  const rf = await handleSocialQueueAction(req({ action: 'post', id: 'a::devto' }),
    { SIGNUP_KV: kv, DEVTO_API_KEY: 'k', DEVTO_ORG_ID: 'o' }, { kv, authorize: superAuth, adapters: failing });
  assert.equal(rf.status, 502);
  assert.equal((await getTask(kv, 'a::devto')).status, 'pending');
  // a done task cannot re-post
  const rd = await handleSocialQueueAction(req({ action: 'post', id: 'b::devto' }),
    { SIGNUP_KV: kv, DEVTO_API_KEY: 'k', DEVTO_ORG_ID: 'o' }, { kv, authorize: superAuth });
