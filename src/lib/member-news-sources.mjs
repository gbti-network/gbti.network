...
// ONLY when its project slug is in the admin-owned approval registry AND that slug resolves to a
// PUBLISHED + PUBLIC project still carrying a newsFeed URL. Anything else (revoked, unpublished,
 ...
 * @param {Array<{project?:string}>} approvals rows from house/member-news-sources.yml `approved`
 * @param {Array<{slug?:string,title?:string,author?:string,status?:string,visibility?:string,newsFeed?:string,shortDescription?:string}>} projects plain project frontmatter
 * @returns the combined source list (house first, then approved member feeds)
 */
export function mergeMemberSources(houseSources, approvals, projects) {
  ...
  for (const row of Array.isArray(approvals) ? approvals : []) {
    const slug = typeof row?.project === 'string' ? row.project.trim() : '';
    if (!slug) continue;
    const p = bySlug.get(slug);
    if (!p) continue; // approved but the project is gone: fail closed
    if (p.status !== 'published' || p.visibility !== 'public') continue; // public gallery projects only
