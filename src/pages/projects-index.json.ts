// SOW-031: emits /projects-index.json at build time, the full newest-first project directory the extension's
...
export const GET: APIRoute = async () => {
  const items = await Promise.all(
    (await getCollection('project'))
      .filter(isListed)
      .sort(byNewest)
      .map(async (e) => ({ ...toIndexItem(e, 'project'), ...(await resolveThumb(e.data, 'project')), categoryLabels: breadcrumb(e.data.categories) })),
  );
