// SOW-031: emits /products-index.json at build time, the full newest-first product directory the extension's
...
export const GET: APIRoute = async () => {
  const items = await Promise.all(
    (await getCollection('product'))
      .filter(isListed)
      .sort(byNewest)
      .map(async (e) => ({ ...toIndexItem(e, 'product'), ...(await resolveThumb(e.data, 'product')), categoryLabels: breadcrumb(e.data.categories) })),
  );
