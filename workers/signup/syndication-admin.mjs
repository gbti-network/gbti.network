...
//   GET  /membership/syndication          -> { ok, pending, sent, cancelled, failed }   (admin/superadmin read)
//   POST /membership/syndication/cancel    -> cancel a pending item within the hold window (SUPERADMIN only)
// Both gate fail-closed via the reconcile-written overrides mirror (authorizeAdmin), so a missing/stale mirror or a
// non-admin token is denied. Cancel additionally requires the superadmin role. Pure over injected deps.

import { authorizeAdmin } from './membership-admin.mjs';
import { ROLE } from '../../membership/overrides-core.mjs';
import { canCancel, markCancelled } from '../../membership/syndication-queue.mjs';
import { getItem, putItem, listAll, removeFromPending } from './syndication-store.mjs';

function bucketize(items, now) {
  const out = { pending: [], sent: [], cancelled: [], failed: [] };
  const t = Number(now());
