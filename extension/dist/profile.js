...
`;
  var GbtiSocialQueue = class extends GbtiElement {
    ...
    render() {
      ...
      const paged = filtered.slice(this._page * PAGE_SIZE2, this._page * PAGE_SIZE2 + PAGE_SIZE2);
    ...
    _doneRow(t) {
      return `<div class="row"><span class="src">${esc(SRC_LABEL2[t.source] || t.source || "")}</span>${this._chip(t.channel, "sent")}<span class="ti">${esc(t.title || "(untitled)")}</span><span class="when">${t.doneAt ? esc(fmtDate(t.doneAt)) : ""}</span><button class="btn del" data-del="${esc(t.id)}" type="button">Delete</button></div>`;
    }
    _autoRow(it) {
      ...
      return `<div class="row"><span class="src">${esc(SRC_LABEL2[it.source] || it.source || "")}</span><span class="ti">${title}</span><span class="chans">${chans}</span><span class="when">${it.sentAt ? esc(fmtDate(it.sentAt)) : ""}</span></div>`;
    }
    _assist(id) {
...
`;
  ...
  .nudge a { color:var(--brand); font-weight:600; }
`;
  var GbtiProfileEditor = class extends GbtiElement {
    _loaded = false;
    _loading = false;
    _addingLink = false;
    _saving = false;
    ...
    _gather() {
      ...
      this.$$("[data-link-key]").forEach((inp) => {
        this._model.links[inp.dataset.linkKey] = inp.value;
      });
    }
    render() {
      this._maybeLoad();
      if (!this.client) {
        this.set(this.css(CSS41) + `<div class="nudge">Open this in the GBTI client or extension to edit your profile.</div>`);
        return;
      }
      if (!this._loaded) {
        this.set(this.css(CSS41) + `<section class="sec"><div class="sec-h"><p style="margin:0">Loading your profile…</p></div></section>`);
        return;
      }
      if (!this._signedIn) {
        this.set(this.css(CSS41) + `<div class="nudge">Sign in with the GBTI client to edit your profile. <a href="${SITE16}/membership/">Become a member</a>.</div>`);
        return;
      }
      ...
      this.set(this.css(CSS41) + sections);
      this._wire();
    }
    _identity(m) {
    ...
    _roles(m) {
      const tags = m.roles.map((r, i) => `<span class="tag role">${esc(prettyRole2(r))}<button type="button" data-rm-role="${i}" aria-label="Remove ${esc(r)}">×</button></span>`).join("");
      return `<section class="sec">
    ...
    _saveBar() {
      ...
      const note = this._paid ? "Publishing updates your public profile on gbti.network within a couple of minutes." : "Publishing your profile needs a paid membership. Your changes stage on your fork and publish when you upgrade.";
    ...
    _wire() {
      ...
      this.$$("[data-rm-skill]").forEach((b) => b.addEventListener("click", () => {
        this._gather();
        this._model.skills.splice(Number(b.dataset.rmSkill), 1);
        this.render();
      }));
      this.$$("[data-rm-role]").forEach((b) => b.addEventListener("click", () => {
        this._gather();
        this._model.roles.splice(Number(b.dataset.rmRole), 1);
        this.render();
      }));
      this.$("[data-add-toggle]")?.addEventListener("click", () => {
        this._gather();
        this._addingLink = !this._addingLink;
        this.render();
      });
      this.$$("[data-add-link]").forEach((b) => b.addEventListener("click", () => {
        this._gather();
        this._model.links[b.dataset.addLink] = "";
        this._addingLink = false;
        this.render();
        this.$(`[data-link-key="${b.dataset.addLink}"]`)?.focus();
      }));
      this.$$("[data-rm-link]").forEach((b) => b.addEventListener("click", () => {
        this._gather();
    ...
    async _save() {
      ...
      const path = this._path || void 0;
      try {
        if (this._paid) {
          await this.client.publish({ type: "profile", input, body, path });
          this._msg = "Profile published. It appears on gbti.network in a couple of minutes.";
          this._msgKind = "ok";
        } else {
          await this.client.saveDraft({ type: "profile", input, body, path });
          this._msg = "Saved to your fork. Upgrade to a paid membership to publish it.";
          this._msgKind = "ok";
        }
        this._fm = { ...this._fm, ...input };
      } catch (e) {
        this._msg = e?.message ? `Could not save: ${e.message}` : "Could not save just now. Try again in a moment.";
        this._msgKind = "err";
      }
      this._saving = false;
      this.render();
